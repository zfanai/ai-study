...
def func1():
    ...
    print "input samples filename"
    #filename = 'mnist_train.mat'     # raw_input() # 换成raw_input()可自由输入文件名
    #filename=r"C:\Users\zhoufan\Projects\Python\mnist-data\train-images-idx3-ubyte"
    filename=r"C:\Users\zhoufan\Projects\Python\mnist-data\mnist_uint8.mat"
    sample = sio.loadmat(filename)
    #print sample
    train_x=sample["train_x"]
    train_y=sample["train_y"]
    
    print 'train_x:', train_x, type(train_x), train_x.shape
    print 'train_y:', train_y, type(train_y), train_y.shape
    
    sample = sample["train_x"]
    #print sample
    ...
    print "input lables filename"
    #filename = 'mnist_train_labels.mat'   # raw_input() # 换成raw_input()可自由输入文件名
    filename=r"C:\Users\zhoufan\Projects\Python\mnist-data\train-labels-idx1-ubyte"
    label = sio.loadmat(filename)
    label = label["mnist_train_labels"]
sample=None
label=None
from demo import load_mnist
sample,label=load_mnist(r'C:\Users\zhoufan\Projects\Python\mnist-data')
print sample,label
#sample *= 2.0
m,n=sample.shape[:]
print m,n
...
#  测试样本， 和目标结果
test_s,test_l=load_mnist(r'C:\Users\zhoufan\Projects\Python\mnist-data')
